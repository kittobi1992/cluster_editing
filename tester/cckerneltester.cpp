...
void solveForConnectedComponent(CostsGraph &G, short pi_type, bool merging, bool just_max, bool split, double &costs, bool unweighted);
		...
		try {
			...
			if (!test) {
				...
				solveForConnectedComponent(CG, 2, merging, just_max, split, costs2, true);
			        if (costs2 != costs) {
					std::cout << "ERROR: found difference -> unweighted=" << costs2 << " weighted=" << costs << std::endl;
