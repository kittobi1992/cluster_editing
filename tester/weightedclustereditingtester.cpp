...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < runs; i++) {
        try {
            ...
            cout << "-----------------------------------" << endl;
            createTesterGraph(CG, mue_ex, sigma_ex, mue_in, sigma_in, !test, unweigthed, cr_fname, k, Graph_size,
                              RandomGraph, k2);
            if (!test) {
                ...
                CG.costsIOOperation("current.cm", MatrixParser::toMatrixFile);
                WeightedProblemInstance PI_B = WeightedProblemInstance(CG, 1E12, false);
                std::cout << "get upper bound.." << std::endl;
                double lb = PI_B.getLowerBound();
                double upper_bound = PI_B.getUpperBound();
                std::cout << "reduce with upb = " << upper_bound << " & lb=" << lb << std::endl;
                WeightedProblemInstance PI_R = WeightedProblemInstance(CG, upper_bound);
                int count = PI_R.maxReduce();
                double red_costs = upper_bound - PI_R.getParameter();
                std::cout << "pidr red costs=" << red_costs << std::endl;

                WeightedProblemInstance *PI = new WeightedProblemInstance(CG, 1E12, false);
                double par = PI->getLowerBound();
                double upper_bound2 = PI->getUpperBound();
                ...
                // test
                if (red_costs - 0.0001 > costs || upper_bound + 0.0001 < costs) {
                    std::cout << "ERROR: " << " costs=" << costs << " redcosts=" << red_costs << " upbound="
...
void solveForConnectedComponent(CostsGraph &G, short pi_type, bool merging, bool just_max, bool split, double &costs) {
    ...
    /*get lower bound*/
    WeightedProblemInstance *PI = new WeightedProblemInstance(G, 1E12, false);
    double par = PI->getLowerBound();
    ...
    while (done == 0) {
        try {
            ...
            std::cout << "search..size: " << (PI->getGraph()).getSize() << std::endl;
            // SEARCH TREE !!!
...
void createRandomGraph2(CostsGraph &CG, bool save_cr, bool unweigthed, string cr_fname, int Graph_size) {
    ...
    if (Graph_size == 0) {
        node = rand() % 100 + 10;
    } else if (Graph_size < 0) {
        node = rand() % (-1 * Graph_size - 500) + 500;
    } else {
        node = Graph_size;
    }
    cout << "Anzahl der Konten: " << node << endl;
    ...
    while (argc > g) {
        if (argv[g][0] == '-') {
            ...
            if (flag == "-output_path" || flag == "O") {
                ...
                cout << "-----------------------------------" << endl;
                cout
                        << "You can create a random CostGraph and write a *.cm and *.cr file, or test the Cluster Editing Tool with a random Graph."
                        << endl;
                cout << "--------------------" << endl;
                ...
                cout
                        << " --size if you want to set the number of nodes in the Graph to a specific value. Default: random number of nodes, 10 to 100."
                        << endl;
                cout << "   Note, if you use a negative value '-a', you'll get random graphs from a to 100" << endl;
                cout
                        << " --runs to specify the number of runs. You shoud not specify a filename, if you want to create more than on Random CostsGraph. \n Must be unsigned int, else default. Default: 1"
                        << endl;
                cout << " --unweighted OR -U If you want to work with an unweighted graph. Default: weighted graph"
                     << endl;
