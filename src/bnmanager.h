    ...
    static inline double getMinBNEdge(int &i, int &j, const CostsGraph *graph, const EdgeReduction *edge_reduction) {
        ...
        for (int h = 0; h < graph->getSize(); h++) {
            for (int k = 0; k < h; k++) {
                ...
                double bn = getLogBN(graph->getEdge(h, k) + 0.000001, edge_reduction->getMinInsertCosts(h, k) -
                                                                      ((graph->getEdge(h, k) > 0) ? 0 : -1 *
                                                                                                        graph->getEdge(
                                                                                                                h, k)),
                                     graph->permanent);
...
private:
    ...
    static inline double getLogBN(double a, double b, double permanent) {
        ...
        if (x <= 10.0) {
            y = (9.12524 + x * (15.9154 + x * (2.69316 + x * 0.0157118))) /
                (4.13278 + x * (21.9904 + x * (12.911 + x)));
        } else {
            y = (5.25471 + x * (74.3963 + x * (3.67704 + x * 0.00435418))) /
                (-87.1522 + x * (120.572 + x * (41.0281 + x)));
        }
