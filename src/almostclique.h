...
public:
    ...
    inline static bool almostCliqueRule(CostsGraph::byte_vector_type clique, const CostsGraph &graph, double parameter,
                                        bool parameter_dependent, std::vector<Array<double> > &triple_list) {
        int size = graph.getSize();
        if (clique.size() > 1) {
