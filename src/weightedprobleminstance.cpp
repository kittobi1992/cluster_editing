...
WeightedProblemInstance::WeightedProblemInstance(CostsGraph graph, double parameter, bool para_independent) : permanent(graph.permanent), forbidden(graph.forbidden), _parameter(parameter), _start_parameter(parameter), _changed_edges(0), _graph(graph), _edge_reduction(), _pid(para_independent)
{
	if (!_pid) {
...
// copy constructor
WeightedProblemInstance::WeightedProblemInstance(const WeightedProblemInstance& PI) : _graph(PI._graph), permanent(PI.permanent), forbidden(PI.forbidden), _parameter(PI._parameter), _start_parameter(PI._start_parameter),  _changed_edges(PI._changed_edges), _edge_reduction(), _pid(PI._pid)
{
	if (!_pid) {
...
WeightedProblemInstance::WeightedProblemInstance(CostsGraph &graph, double parameter, double start_parameter,  EdgeReduction edge_reduction, bool para_independent) : _graph(graph), permanent(graph.permanent), forbidden(graph.forbidden), _parameter(parameter), _start_parameter(start_parameter), _changed_edges(0), _pid(para_independent)
{
	if (!_pid) {
...
{	
	...
	if (permanent_list.size() > 0) {

		for (int i = 0; i < permanent_list.size(); i++) {
		     if (permanent_list[i].i != permanent_list[i].j && _graph.getEdge(permanent_list[i].i, permanent_list[i].j) != forbidden) {
...
inline int WeightedProblemInstance::simpleReduction()
{
	...
	std::vector< pair_type > forbidden_list = std::vector< pair_type >(0);
	...
	for (int i = 0; i < size; i++) {
		for (int k = 0; k < i; k++) {
		     ...
		     if (edge < 0 && edge != forbidden) {
		          ...
		          for (int h = 0; h < k; h++) {
		               sum1 += (_graph.getEdge(i,h)>0) ? _graph.getEdge(i,h) : 0;
		               sum2 += (_graph.getEdge(k,h)>0) ? _graph.getEdge(k,h) : 0;
		          }
		          for (int h = k + 1; h < i; h++) {
		               sum1 += (_graph.getEdge(i,h)>0) ? _graph.getEdge(i,h) : 0;
		               sum2 += (_graph.getEdge(k,h)>0) ? _graph.getEdge(k,h) : 0;
		          ...
		          if (sum1 < -1 * edge || sum2 < -1 * edge) {
		               pair_type e = {i, k};
		               forbidden_list.insert(forbidden_list.end(), e);
		          }
		          ...
		          double sum3 = 0.0;
		          for (int h = 0; h < k; h++) {
		               sum1 += (_graph.getEdge(i,h)>0) ? _graph.getEdge(i,h) : (_graph.getEdge(i,h) * -1);
		               sum2 += (_graph.getEdge(k,h)>0) ? _graph.getEdge(k,h) : (_graph.getEdge(k,h) * -1);
		               sum3 += (_graph.getEdge(i,h)>0) ? _graph.getEdge(i,h) : 0;
		               sum3 += (_graph.getEdge(k,h)>0) ? _graph.getEdge(k,h) : 0;
		          }
		          for (int h = k + 1; h < i; h++) {
		               sum1 += (_graph.getEdge(i,h)>0) ? _graph.getEdge(i,h) : (_graph.getEdge(i,h) * -1);
		               sum2 += (_graph.getEdge(k,h)>0) ? _graph.getEdge(k,h) : (_graph.getEdge(k,h) * -1);
		          ...
		          if (sum3 < edge) {
		               pair_type e = {i, k};
		               permanent_list.insert(permanent_list.end(), e);
			  }
...
inline int WeightedProblemInstance::criticalCliqueReduction(int max_edge, bool check_for_int)
{
	...
	if (check_for_int) {
		...
		int zero = static_cast<int>((fabs(_graph.getEdge(1,0))*1 - static_cast<int>(fabs(_graph.getEdge(1,0)) * 1))*10);
		integer_instance = (zero == 0 ) ? true : false;
			...
			for (int i=0; i<_graph.getSize();i++) {	
				for (int k=0; k < i;k++) {
					if (_graph.getEdge(i,k) != _graph.forbidden) {
						if (zero != static_cast<int>((fabs(_graph.getEdge(i,k))*1 - static_cast<int>(fabs(_graph.getEdge(i,k)) * 1))*10)) {
...
inline int WeightedProblemInstance::detectCliques()
{
	...
	for (int i = 0; i < cliques.size(); i++) {
		...
		for (int k = i + 1; k < cliques.size(); k++) {
		     ...
		     for (int h = 0; h < cliques[k].size(); h++) {
		         for (CostsGraph::byte_vector_type::iterator l = cliques[k].end()-1; l != cliques[k].begin()+h; l--) {
		              if (*l == cliques[k][h]) {
		                   cliques[k].erase(l);
		              }
		         }
...
inline void WeightedProblemInstance::saveChangedEdge(int i, int j, double costs, char operation) 
{
	...
	_changed_edges.insert(_changed_edges.end(), new_edge);
}
...
int WeightedProblemInstance::reduce()
{
	...
	if (!_pid) {
		CostsGraph::edge_list_type permanent_list = CostsGraph::edge_list_type(0);
		CostsGraph::edge_list_type forbidden_list = CostsGraph::edge_list_type(0);
...
int WeightedProblemInstance::ccReduce()
{
	...
	for (int i = 0; i < _graph.getSize(); i++) {
	     for (int k = 0; k < i; k ++) {
	         valid = (fabs(_graph.getEdge(i,k)) == 1.0) ? true : false;
	     }
	}
...
WeightedProblemInstance::pi_list_type WeightedProblemInstance::divideInstance(CostsGraph::vertex_matrix_type vertex_matrix)
{
	// build new graphs
