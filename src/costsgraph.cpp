...
CostsGraph::CostsGraph(int size, double th ) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max()* -1), permanent(std::numeric_limits<double>::max()), _cost_matrix(size, forbidden)
{
...
CostsGraph::CostsGraph(int size, std::string* edge_list, double th ) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
...
// constructor with size, threshold and list of names
CostsGraph::CostsGraph(int size, vertex_name_type edge_list, double th ) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()),  _cost_matrix(size, forbidden)
{
...
CostsGraph::CostsGraph(int size, double_matrix_type costs, double th)  : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()),  _cost_matrix(size, forbidden)
{
...
CostsGraph::CostsGraph(int size, double_matrix_type costs, vertex_name_type edge_list, double th ) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix(size, forbidden)
{
	// throw error if weight matrix has wrong size
	if (costs.size() != _size) {
		throw GraphException( "costs matrix has no valid format");
...
CostsGraph::CostsGraph(int size, double_matrix_type weight_matrix, costs_parsing_fct_type cost_fct, double th) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix(size, forbidden)
{
	// throw error if weight matrix has wrong size
...
// which the weights will be converted to costs
CostsGraph::CostsGraph(int size, double_matrix_type weight_matrix, costs_parsing_fct_type cost_fct, vertex_name_type edge_list, double th) : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix(size, forbidden)
{
	// throw error if weight matrix has wrong size
	if (weight_matrix.size() != _size) {
		throw GraphException( "weight matrix has no valid format");
...
CostsGraph::CostsGraph(char* file_name, double_file_fct_type fct, costs_parsing_fct_type cost_fct, double th ) : _size(0), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
	try{
...
// the file will be parsed with the double file fct and every read weight value will be converted
// to costs with the costs parsing function using the threshold
CostsGraph::CostsGraph(std::string file_name, double_file_fct_type fct, costs_parsing_fct_type cost_fct, double th ) : _size(0), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
	try{
...
CostsGraph::CostsGraph(char* file_name, matrix_file_fct_type2 fct, costs_parsing_fct_type cost_fct, double th) : _size(0),
_threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
	if (th < 0) {
...
// constructor like above but reads in a matrix file which needs still cost parsing
CostsGraph::CostsGraph(std::string file_name, matrix_file_fct_type2 fct, costs_parsing_fct_type cost_fct, double th) : _size(0),
_threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
	try{
		char* str = strdup(file_name.c_str());
...
CostsGraph::CostsGraph(char* file_name, matrix_file_fct_type fct) : _size(0), _threshold(1e-20), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()),  _cost_matrix()
{
	fct(file_name, *this);
}
...
// constructor with file name and a matrix file function, which just reads the raw matrix as cost graph
// the threshold is no longer important...all other infos like names will also be read from file
CostsGraph::CostsGraph(std::string file_name, matrix_file_fct_type fct): _size(0), _threshold(1e-20), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix()
{
	char* str = strdup(file_name.c_str());
	fct(str, *this);
}
...
// cop constructor
CostsGraph::CostsGraph(CostsGraph const &new_graph) : _size(new_graph._size),  forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()), _cost_matrix(new_graph._cost_matrix)
{
...
{	
	...
	_info_list[intern_index1].names.insert(_info_list[intern_index1].names.end(), _info_list[intern_index2].names.begin(), _info_list[intern_index2].names.end());	
	_info_list[intern_index1].indices.insert(_info_list[intern_index1].indices.end(), _info_list[intern_index2].indices.begin(), _info_list[intern_index2].indices.end());	
