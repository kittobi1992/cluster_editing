...
EdgeReduction::EdgeReduction(CostsGraph *graph, double parameter) : _graph(graph), _parameter(parameter),
                                                                    _min_insert_costs(_graph->getSize(), 0),
                                                                    _min_delete_costs(_graph->getSize(), 0),
...
// copy constructor
EdgeReduction::EdgeReduction(const EdgeReduction &ER, CostsGraph *graph) : _graph(graph), _parameter(ER._parameter),
...
inline EdgeReduction::EdgeReduction(CostsGraph *graph, double parameter, triangle_matrix_type m_i_c,
                                    triangle_matrix_type m_d_c) : _graph(graph), _parameter(parameter),
                                                                  _min_insert_costs(m_i_c), _min_delete_costs(m_d_c),
                                                                  forbidden(_graph->forbidden),
                                                                  permanent(_graph->permanent) {
}
...
inline void EdgeReduction::createCostsMatrices() {
    ...
    for (int i = 0; i < size; i++) {
        for (int k = 0; k < i; k++) {
            ...
            _min_insert_costs.dirpos(i, k) =
                    sum_insert + ((_graph->getEdge(i, k) <= 0) ? (_graph->getEdge(i, k) * -1) : 0);
            _min_delete_costs.dirpos(i, k) = sum_delete + ((_graph->getEdge(i, k) > 0) ? _graph->getEdge(i, k) : 0);

            if (_min_insert_costs.dirpos(i, k) > _parameter + 0.01 &&
                _min_delete_costs.dirpos(i, k) > _parameter + 0.01) {
    ...
    if (old_value > 0 && value <= 0) {
        ...
        for (int k = 0; k < _graph->getSize(); k++) {
            ...
            if (k != i && k != j) {
                ...
                if (edge1 > 0 && edge2 > 0) {
                    updateEdgeReference(j, k, 'i',
                                        getMinDeleteCosts(j, k) - minDelEdge(old_value, _graph->getEdge(i, k)));
                    updateEdgeReference(j, k, 'd', getMinInsertCosts(j, k) + _graph->getEdge(i, k));
                    updateEdgeReference(i, k, 'i',
                                        getMinDeleteCosts(i, k) - minDelEdge(old_value, _graph->getEdge(j, k)));
                    ...
                    updateEdgeReference(j, k, 'i',
                                        getMinDeleteCosts(j, k) - minDelEdge(old_value, _graph->getEdge(i, k)));
                    updateEdgeReference(j, k, 'd', getMinInsertCosts(j, k) + _graph->getEdge(i, k));
                    updateEdgeReference(i, k, 'd',
                                        getMinInsertCosts(i, k) - minInDelEdge(_graph->getEdge(j, k), old_value));
                } else if (edge1 <= 0 && edge2 > 0) {
                    // case 2.3: k is neighbor of i but not of j
                    updateEdgeReference(i, k, 'i',
                                        getMinDeleteCosts(i, k) - minDelEdge(old_value, _graph->getEdge(j, k)));
                    updateEdgeReference(i, k, 'd', getMinInsertCosts(i, k) + _graph->getEdge(j, k));
                    updateEdgeReference(j, k, 'd',
                                        getMinInsertCosts(j, k) - minInDelEdge(_graph->getEdge(i, k), old_value));
                } else {
...
inline void
EdgeReduction::updateEdgeCase1(int i, int j, int a, int b, double new_costs1, double new_costs2, double &up_insert,
                               double &up_delete) {
    double old_insert = 0;
    double old_delete = 0;
...
// and (b,j)
inline void
EdgeReduction::updateEdgeCase2(int i, int j, int a, int b, double new_costs1, double new_costs2, double &up_insert,
                               double &up_delete) {
    double new_insert = 0;
    double new_delete = 0;
...
inline void EdgeReduction::updateCostsMatrices(int a, int b, const double_array_type &new_costs) {
    ...
    for (int i = 0; i < b; i++) {
        for (int k = 0; k < i; k++) {
            // update (i,k)
            updateEdgeCase1(i, k, a, b, new_costs[i], new_costs[k], _min_insert_costs.dirpos(i, k),
                            _min_delete_costs.dirpos(i, k));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i], new_costs[k], _min_insert_costs.dirpos(b, i),
                            _min_delete_costs.dirpos(b, i));
            updateEdgeCase2(k, i, a, b, new_costs[k], new_costs[i], _min_insert_costs.dirpos(b, k),
                            _min_delete_costs.dirpos(b, k));
        }
    }
    for (int i = b + 1; i < a; i++) {
        for (int k = 0; k < b; k++) {
            // update i,k
            updateEdgeCase1(i, k, a, b, new_costs[i - 1], new_costs[k], _min_insert_costs.dirpos(i, k),
                            _min_delete_costs.dirpos(i, k));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i - 1], new_costs[k], _min_insert_costs.dirpos(i, b),
                            _min_delete_costs.dirpos(i, b));
            updateEdgeCase2(k, i, a, b, new_costs[k], new_costs[i - 1], _min_insert_costs.dirpos(b, k),
                            _min_delete_costs.dirpos(b, k));
        }
        for (int k = b + 1; k < i; k++) {
            // update i,k
            updateEdgeCase1(i, k, a, b, new_costs[i - 1], new_costs[k - 1], _min_insert_costs.dirpos(i, k),
                            _min_delete_costs.dirpos(i, k));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i - 1], new_costs[k - 1], _min_insert_costs.dirpos(i, b),
                            _min_delete_costs.dirpos(i, b));
            updateEdgeCase2(k, i, a, b, new_costs[k - 1], new_costs[i - 1], _min_insert_costs.dirpos(k, b),
                            _min_delete_costs.dirpos(k, b));
        }
    }
    for (int i = a + 1; i < size; i++) {
        for (int k = 0; k < b; k++) {
            // update i,k
            updateEdgeCase1(i, k, a, b, new_costs[i - 2], new_costs[k], _min_insert_costs.dirpos(i - 1, k),
                            _min_delete_costs.dirpos(i - 1, k));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i - 2], new_costs[k], _min_insert_costs.dirpos(i - 1, b),
                            _min_delete_costs.dirpos(i - 1, b));
            updateEdgeCase2(k, i, a, b, new_costs[k], new_costs[i - 2], _min_insert_costs.dirpos(b, k),
                            _min_delete_costs.dirpos(b, k));
        }
        for (int k = b + 1; k < a; k++) {
            // update i,k
            updateEdgeCase1(i, k, a, b, new_costs[i - 2], new_costs[k - 1], _min_insert_costs.dirpos(i - 1, k),
                            _min_delete_costs.dirpos(i - 1, k));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i - 2], new_costs[k - 1], _min_insert_costs.dirpos(i - 1, b),
                            _min_delete_costs.dirpos(i - 1, b));
            updateEdgeCase2(k, i, a, b, new_costs[k - 1], new_costs[i - 2], _min_insert_costs.dirpos(k, b),
                            _min_delete_costs.dirpos(k, b));
        }
        for (int k = a + 1; k < i; k++) {
            // update i,k
            updateEdgeCase1(i, k, a, b, new_costs[i - 2], new_costs[k - 2], _min_insert_costs.dirpos(i - 1, k - 1),
                            _min_delete_costs.dirpos(i - 1, k - 1));
            // update i,ab
            updateEdgeCase2(i, k, a, b, new_costs[i - 2], new_costs[k - 2], _min_insert_costs.dirpos(i - 1, b),
                            _min_delete_costs.dirpos(i - 1, b));
            updateEdgeCase2(k, i, a, b, new_costs[k - 2], new_costs[i - 2], _min_insert_costs.dirpos(k - 1, b),
                            _min_delete_costs.dirpos(k - 1, b));
        }
    }
}
...
bool EdgeReduction::getReduceableEdges(CostsGraph::edge_list_type &permanent_edges,
                                       CostsGraph::edge_list_type &forbidden_edges, double fuzzy_threshold) {
...
EdgeReduction::edge_red_list_type
EdgeReduction::divideInstance(CostsGraph::vertex_matrix_type vertex_matrix, CostsGraph::graph_list_type graph_list) {
