...
void fillParameters(int argc, char **argv, std::string &fname, char *&outputfile, char *&output_fname, bool &names,
                    double &scale);
            ...
            for (int k = 0; k < i; k++) {
                ...
                if (G.getEdge(i, k) > 0) {
                    ...
                    if (getComp(i, cc) == getComp(k, cc)) {
                        graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#" << buffer << buffer << buffer
                                   << "\", len=" << (0.8 * scale) << "];" << std::endl;
                    } else {
                        graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#" << buffer << buffer << buffer
                                   << "\"];" << std::endl;
                    }
                    ...
                    if (G.getEdge(i, k) / min < 0.1) {
                        graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#d7d7d7\", style=\"dotted\", len=" << n
                                   << "];" << std::endl;
                    } else {
                        graph_file << "\t" << i_n << " -- " << k_n << " [color=\"transparent\", style=\"dotted\", len="
                                   << n << "];" << std::endl;
                    }
                }
            }
...
void fillParameters(int argc, char **argv, std::string &fname, char *&outputfile, char *&output_fname, bool &names,
    ...
    while (argc > g) {
        ...
        if (flag == "--help") {
            ...
            std::cout
                    << "Usage: graphvizcreator [cm-file] [clustering-output-file] [output-file] [0=number,1=names] [scaling factor]"
                    << std::endl << std::endl;
            std::cout
                    << "Creates a file for the graph visualization\n software GraphViz from the given\n *.cm file and the resulting clustering output file. "
                    << std::endl << std::endl;
            exit(1);
        } else if (g == 1) {
            fname = argv[g];
