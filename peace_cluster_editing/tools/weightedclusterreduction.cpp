...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (int i = 0; i < old_size; i++) {
            for (int k = 0; k < i; k++) {
                if (G.getEdge(i, k) > G.forbidden) {
                    sum_edges += (G.getEdge(i, k) > 0) ? G.getEdge(i, k) : -1 * G.getEdge(i, k);
                    edges++;
                }
            }
        }
        ...
        double lower_bound = PI_B.getLowerBound();
        std::cout << "upper bound: " << upper_bound << std::endl;
        std::cout << "lower bound: " << lower_bound << std::endl;
        std::cout << "avg.weight: " << (sum_edges / edges) << std::endl;
        std::cout << "create instance" << std::endl;
        WeightedProblemInstance PI = WeightedProblemInstance(G, upper_bound);
        ...
        std::cout << "  Reduction in " << used_time << " sec caused " << count << " edge changes. Graph reduced from "
                  << old_size << " to " << (PI.getGraph()).getSize() << std::endl;
        std::cout << "  Modification costs are " << (upper_bound - PI.getParameter()) << std::endl;
        ...
        std::cout << "  Instance splitted in " << graph_set.getSetSize() << " components. "
                  << "Greates component of size: " << max_size << std::endl;
...
void fillParameters(int argc, char **argv, std::string &fname, char *&output_fname) {
    ...
    while (argc > g) {
        if (argv[g][0] == '-') {
            ...
            flag = flag.erase(0, 1);
            if (flag == "-help") {
                std::cout << "Weighted Cluster Editing Parameter Independent Reducer " << std::endl;
                std::cout << "--------------------" << std::endl;
