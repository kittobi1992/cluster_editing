    ...
    for (int i = 0; i < runs; i++) {
        try {
            ...
            if (!test) {
                ...
                //std::cout << "Graph: " << CG << std::endl;
                double costs = 0;
                solveForConnectedComponent(CG, 2, merging, just_max, split, costs, false);
                double costs2 = 0;
                solveForConnectedComponent(CG, 2, merging, just_max, split, costs2, true);
                if (costs2 != costs) {
                    std::cout << "ERROR: found difference -> unweighted=" << costs2 << " weighted=" << costs
                              << std::endl;
...
void solveForConnectedComponent(CostsGraph &G, short pi_type, bool merging, bool just_max, bool split, double &costs,
    ...
    /*get lower bound*/
    PI = new WeightedProblemInstance(G, 1E12, true);
    ...
    while (done == 0) {
        try {
            std::cout << "Component " << "(size=" << G.getSize() << "): try with costs = " << par << std::endl;
            par += par_steps;
            PI = new WeightedProblemInstance(G, par - par_steps);

            int count = 0;
            if (unweighted) {
                count += PI->ccKernelization();
            }
            count += PI->maxReduce();
            // SEARCH TREE !!!
