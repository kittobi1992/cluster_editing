...
#include "cluster_editing/utils/logger.h"
...
#define LOGCC(cond, newline) \
  !(cond) ? (void)0 :        \
  cluster_editing::LoggerVoidify() & cluster_editing::Logger(newline)

// ! Explicitly specify the condition for logging
#define DBGCC(cond, newline) \
  !(cond) ? (void)0 :        \
  cluster_editing::LoggerVoidify() & cluster_editing::Logger(newline, __FILENAME__, __FUNCTION__, __LINE__)
...
#define ASSERT_MSG(msg) LOG << RED << "[ASSERTION]" << END << msg; std::exit(-1)
