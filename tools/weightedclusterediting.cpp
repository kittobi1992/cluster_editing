...
void fillParameters(int argc, char **argv, std::string &fname, char* &output_fname, GraphSet::costs_parsing_fct_type &parsing_fct, GraphSet::graph_set_parser_fct_type &read_graph_fct, double &threshold, double &parameter, int &parameter_steps, short &pi_type,  int &max_component, bool &merging, short &mode, bool &just_max, bool &split, bool &loginfo, char* &log_fname);
...
void writeCalculatedSolutions(int nr, SearchTreeWeighted::solutions_type solutions, std::ofstream &log_file, std::ofstream &output_file, bool &loginfo);
...
int main(int argc, char **argv)
{
	...
	fillParameters(argc, argv, fname, output_fname, parsing_fct, read_graph_fct, threshold, parameter, parameter_steps, pi_type, max_component, merging, mode, just_max, split, loginfo, log_fname);
	...
	try {
		     ...
		     for (int i = 0; i < graph_set.getSetSize(); i++ ) {
		        ...
		        if (G.getSize() <= max_component) {
		             ...
		             double costs = 0;
		             //double this_time =0;
		             double this_time = solveForConnectedComponent(G, i, min_parameter, solutions_nr, parameter, parameter_steps, log_file, pi_type, merging, just_max, split, costs, output_file, loginfo);
		             used_time += this_time;
	...
	for (int k = 0; k < solutions.size(); k++) {
		...
		if (changed.size() == 0) {
		     ...
		     switch (changed[0].operation) {
		               case 'a' : if(loginfo)log_file << "add"; break;
		               case 'd' : if(loginfo)log_file << "del"; break;
		               case 'm' : if(loginfo)log_file << "merge"; break;
		               default: break;
		          }
		     if(loginfo)log_file << "(";
...
double solveForConnectedComponent(CostsGraph G, int nr, double &min_parameter, long &solutions_nr, double parameter, int parameter_steps, std::ofstream &log_file, short pi_type, bool merging, bool just_max, bool split, double &costs, std::ofstream &output_file, bool &loginfo)
{
	...
	while( done == 0 ) {
		try {
		     ...
		     std::cout << " Reduction with maximum costs " << par-par_steps << " caused " << count << " edge changes with costs " << par-PI->getParameter()-par_steps << " new graph size is " << (PI->getGraph()).getSize() << std::endl;
...
//		          g--;
		          ...
		          std::cout << " --mode OR -X <1|2|3> \tIndicates input mode\n\t\t\t\t1 for edge files\n\t\t\t\t2 for matrix files\n\t\t\t\t3 for parsing a directory with connected \n\t\t\t\tcomponents in matrix format\n\t\t\t\tDefault: 1" << std::endl;
		          std::cout << " --threshold OR -T <REAL_VALUE>	Indicates threshold for edges" << std::endl << "\t\t\t\tDefault: 1e-40" << std::endl;
		          std::cout << " --parameter OR -P <INTEGER_VALUE>\tIndicates parameter, 0 for \n\t\t\t\tsearch for parameter by increasement" << std::endl << "\t\t\t\tDefault: 0 (search for parameter by \n\t\t\t\tincreasement)" << std::endl;
		          std::cout << " --parameter_steps OR -S <INTEGER_VALUE>\tIndicates step size for \n\t\t\t\tparameter increasement \n\t\t\t\t 0 for automatic calculation of step size" << std::endl << "\t\t\t\tDefault: 0" << std::endl;
		          std::cout << " --unweighted OR -U \tUse on unweighted graph" << std::endl << "\t\t\t\tDefault: weighted graph" << std::endl;
		          std::cout << " --max_component OR -M <INTEGER_VALUE> \tIndicates maximum size \n\t\t\t\t of components considered in the calculation \n\t\t\t\tDefault: 32767" << std::endl;
		          std::cout << " --costparser OR -C <COST_PARSER>\tIndicates which cost parser will be \n\t\t\t\tused for transforming edge weights into costs" << std::endl << "\t\t\t\tOptions: BlastParser.exponentToCosts, \n\t\t\t\t BlastParser.simpleCutOff" << std::endl << "\t\t\t\tDefault: BlastParser.exponentToCosts" << std::endl;
		          std::cout << " --graphparser OR -G <GRAPH_PARSER>\tIndicates which graph parser will be \n\t\t\t\tused to read an edge file" << std::endl << "\t\t\t\tOptions: EdgeFileParser.initFrom5ColumnFile,\n\t\t\t\t EdgeFileParser.initFrom3ColumnFile, \n\t\t\t\t EdgeFileParser.initFrom12ColumnFile" << std::endl << "\t\t\t\tDefault: EdgeFileParser.initFrom5ColumnFile" << std::endl;
		          std::cout << " --all_solutions OR -A\tReturning all possible solutions\n\t\t\t\t(Note: not compatible with option -s) \n\t\t\t\tDefault: returning only the best solution" << std::endl;
		          std::cout << "DISABLE SPEED-UP OPTIONS" << std::endl;
		          std::cout << " --no_split OR -s\tDisable splitting of search tree\n\t\t\t\tfor connected components\n\t\t\t\tNote: Returns just optimal solutions\n\t\t\t\tDefault: Splitting enabled" << std::endl;
		          exit(0);
		     } else {
