	...
	try {
		     ...
		     for (int i = 0; i < graph_set.getSetSize(); i++ ) {
		        ...
		        while( done == 0 ) {
				  ...
				  std::cout << G << std::endl;
		                  int count = (*PI).reduce();
		                  std::cout << " with parameter " << par-par_add << " caused " << count << " changes. Parameter was reduced by " << par-(*PI).getParameter()-par_add << std::endl;
		                  ...
		                  if (solutions.size() > 0) {
		                       ...
		                       for (int k = 0; k < solutions.size(); k++) {
		                            ...
		                            for (int i = 0; i < changed.size(); i++) {
		                                 std::cout << "(" << changed[i].i << "," << changed[i].j << ") costs: " << changed[i].costs << " ";
		                            }
		                            std::cout << std::endl;
		                            std::cout << "with costs" << (*solutions[k]).getStartParameter() - (*solutions[k]).getParameter() - 1 << std::endl;
		                       }
