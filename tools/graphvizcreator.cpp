...
void fillParameters(int argc, char **argv, std::string &fname, char* &outputfile, char* &output_fname, bool &names, double &scale);
...
int main(int argc, char **argv)
{
	...
	fillParameters(argc, argv, fname, outputfile, output_fname, names, scale);
	...
	try {
		     ...
		     readOutputFile(outputfile, cc, G);
		     ...
		     graph_file << "graph G {" << std::endl;
		     graph_file << "\tsize=\"30,15\";" << std::endl;
		     graph_file << "\tgraph [splines=false overlap=scale];" << std::endl;		    
		     for (int i = 0; i < G.getSize(); i++) {
		          for (int k = 0; k < i; k++) {
		               std::string i_n = G.getVertexName(i);
		               std::string k_n = G.getVertexName(k);
		               for (int j=0; j < i_n.size(); j++) {
		                    if (i_n[j]=='_' || i_n[j]=='.') i_n[j] = 'X';
		               } 
		               for (int j=0; j < k_n.size(); j++) {
		                    if (k_n[j]=='_' || k_n[j]=='.') k_n[j] = 'X';
		               } 
		               if (!n) {
		                    i_n = toString(i);
		                    k_n = toString(k);
		               ...
		               if (G.getEdge(i,k) > 0) {
		                    ...
		                    if (getComp(i, cc) == getComp(k, cc)) {
		                          graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#" << buffer << buffer << buffer << "\", len="<< (0.8*scale) << "];" << std::endl;
		                    } else {
		                          graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#" << buffer << buffer << buffer << "\"];" << std::endl;
		                    }
		               } else {
		                    double n = (ceil(G.getEdge(i,k)/min * 100+20))*scale;
		                    if (G.getEdge(i,k)/min < 0.1) {
		                         graph_file << "\t" << i_n << " -- " << k_n << " [color=\"#d7d7d7\", style=\"dotted\", len=" << n << "];" << std::endl;
		                    } else {
		                         graph_file << "\t" << i_n << " -- " << k_n << " [color=\"transparent\", style=\"dotted\", len=" << n << "];" << std::endl;
		                    }
		               }
		          }
		     }
...
void fillParameters(int argc, char **argv, std::string &fname, char* &outputfile, char* &output_fname, bool &names, double &scale)
{
	...
	while( argc > g ) {
		...
		if (flag == "--help") {
			...
			std::cout << "Usage: graphvizcreator [cm-file] [clustering-output-file] [output-file] [0=number,1=names] [scaling factor]" << std::endl << std::endl;
			std::cout << "Creates a file for the graph visualization\n software GraphViz from the given\n *.cm file and the resulting clustering output file. " << std::endl << std::endl;
			exit(1);
		} else if (g == 1) {
			fname = argv[g];
			g++;
